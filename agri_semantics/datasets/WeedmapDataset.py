...
class WeedmapDataModule(LightningDataModule):
    ...
    def setup(self, stage: str = None):
        ...
        if stage == "fit" or stage is None:
            ...
            if self.active_learning and self.all_indices is None:
                train_indices = np.arange(len(self._weedmap_train))
                max_num_indices = min(len(self._weedmap_train), self.max_collected_images)
                self.all_indices = np.sort(np.random.choice(train_indices, size=max_num_indices, replace=False))

        # Assign test dataset for use in dataloader(s)
        if stage == "test" or stage is None:
            self._weedmap_test = WeedmapDataset(
    ...
    def unlabeled_dataloader(self):
        ...
        train_dataset = self._weedmap_train
    ...
    def val_dataloader(self):
        ...
        loader = DataLoader(self._weedmap_val, batch_size=batch_size, num_workers=n_workers, shuffle=False)
    ...
    def test_dataloader(self):
        ...
        loader = DataLoader(self._weedmap_test, batch_size=batch_size, num_workers=n_workers, shuffle=False)
...
class WeedmapDataset(Dataset):
    """Represents the Weedmap dataset.
    ...
    def __getitem__(self, idx: int) -> Dict[str, torch.Tensor]:
        ...
        anno = WeedmapDataset.remap_annotation(anno)
    ...
    def remap_annotation(anno: np.ndarray) -> np.ndarray:
        ...
        # mask = (anno == 10000)
        # remapped[mask] = 1
        remapped = np.zeros((dims[0], dims[1]))
        if len(dims) == 3:
            mask_crop = anno[:, :, 1] == 255
            remapped[mask_crop] = 1
            mask_weed = anno[:, :, 2] == 255
            remapped[mask_weed] = 2
